...
const puzzleType = require("./tools").puzzleType;
...
var ScrambleImage = function() {
    ...
    const Rotate = utils.ctxRotate;
    const Transform  = utils.ctxTransform;
    const drawPolygon = utils.ctxDrawPolygon;
    ...
    //different from og: type and scramble are separate for convenience (no input type that needs to be accounted for)
    function genImage(type, scramble) {
        function createPNG() {
        ...
        for (size = 0; size < 12; size++) {
            if (type == types_nnn[size]) {
                nnnImage(size, scramble);
				return canvas.toBuffer()
                //createPNG();
                //return true;
            }
        }
        //not necessary for our use, but could be easily added for >11x11
        // if (type == "cubennn") {
